...
class GreenFunctions : public Measurements::MeasurementBase {
public:
            ...
            for(int pos_c = 0; pos_c < conf()->get_flavor_order(flavor_c); ++pos_c){
                ...
                for(int flavor_a = 0; flavor_a < conf()->get_n_flavor(); flavor_a++){
                    for(int pos_a = 0; pos_a < conf()->get_flavor_order(flavor_a); ++pos_a){
                        ...
                        DEBUG( std::cout << "branch " << t_a.branch << ": " << t_a.t << ", bin: " << bin_a << std:: endl;
                        ...
                        if(t_a.branch == IMAG || t_c.branch == IMAG){
                            ...
                            if(t_a > t_c){
                                ...
                                // greater
                                raw_data[0].slice(flavor_a, flavor_c)(bin_a, bin_c) += contribution;
                            }
                            else{
                                DEBUG( std::cout << "<" << std::endl; )
                                // lesser
                                raw_data[1].slice(flavor_a, flavor_c)(bin_a, bin_c) += contribution;
                            }
                        }
                        ++j;
                    }
    ...
    void reset() override {
        auto reset_data = [this](data_t &data){
            ...
            data.clear();
            data.emplace_back(cx_tetracube{n_times(), n_times(), n_flavor, n_flavor});
            data.emplace_back(cx_tetracube{n_times(), n_times(), n_flavor, n_flavor});
    ...
    void print_normalized(std::ostream &os) const override {
        os << "Green functions have been measured." << std::endl;
    }
...
public:
    ...
    void reduce(MeasurementBase &global_m) override {
        auto& m = static_cast<GreenFunctions&>(global_m);
        for(int i = 0; i < 4; ++i){
