...
QMC::QMC(const std::string &name, double beta, double t_max, const InputParams &input_params)
        : bw_type(NUMERIC_BW), lw_type(NUMERIC_LW),
          ...
          bath_weight(model, configuration, bw_type),
          local_weight(model, configuration, lw_type),
...
QMC::QMC(const std::string &name, double beta, double t_max,
        ...
        : bw_type(NUMERIC_BW), lw_type(DISCRETE_BATH),
          model(name, beta, t_max, input_params, model_params, load_delta_up, load_delta_down, load_propagator),
          configuration(t_max, beta, model.n_flavor),
          bath_weight(model, configuration, bw_type),
          local_weight(model, configuration, lw_type),
          moves(model, configuration, local_weight.get(), bath_weight, random_number_generator),
...
void QMC::initialize_configuration_and_weights() {
    // Generating the initial configuration. Default: k = 1 configuration
    configuration.randomly_generate(1, random_number_generator);
...
void QMC::run_measurement(long int N_MC) {
    ...
    if(mpi_rank() == 0)
#endif
        std::cout  << "STARTING MEASUREMENT" << std::endl << std::endl;
    if(!is_measuring){
...
void QMC::run_warm_up(long int N_MC) {
    ...
    if(mpi_rank() == 0)
#endif
        std::cout << "STARTING WARM UP" << std::endl << std::endl;
    if(is_measuring){
        //measurements.reset_measurements();
        is_measuring = false;
    }
...
void QMC::set_n_blocks(int n) {
    if(lw_type != DISCRETE_BATH){
        std::cout << "The mode of local weight evaluation is not DISCRETE_BATH. No action performed. " << std::endl;
        return;
    }
    auto& discrete_bath_weight = dynamic_cast<DiscreteBathWeight&>(local_weight.get());
    discrete_bath_weight.set_n_blocks(n);
}
...
std::ostream &operator<<(std::ostream &os, const QMC &qmc) {
    ...
    os << "*** Non-equilibrium CT-1/2-HYB QMC ***" << std::endl;
    os << std::endl;
    os << qmc.model;
